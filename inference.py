...
from inference_model import inference_SNNCNN3
...
def inference(data_loader, model, weight, result_path, class_names, no_average,
              output_topk, epoch, tb_writer, distributed, device, test_num):
    print('inference', '---', test_num )

    model = inference_SNNCNN3(model)
    model = make_data_parallel(model, distributed, device)
    model.eval()
    ...
    with torch.no_grad():
        for i, (event_inputs, frame_inputs, targets) in enumerate(data_loader):
            ...
            #print('event_inputs', frame_inputs.shape)
            end_time = time.time()
            outputs = model(frame_inputs, event_inputs, weight)
